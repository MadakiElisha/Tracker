...
from pid_controller import TrackerPID
from fc_interface import FCInterface
...
def draw_hud(frame, locked_id, locked_box,
    ...
    mode = t.get('mode', '?')
    hdg  = t.get('heading', 0)
    bat  = t.get('battery', 0.0)
    sats = t.get('gps_sats', 0)
    ...
    if locked_box is not None:
        ...
        fill_frac = min(dbg['current_area'] /
                        max(dbg['setpoint_area'] * 2, 0.001), 1.0)
...
def main():
    ...
    try:
        while not flag_quit.is_set():
            ...
            # --- Draw ---
            frame, locked_box = draw_tracks(
                frame, tracks, locked_id)
            frame = draw_hud(
            ...
            if key == ord('q'):
                ...
                print(f"\n[PID] Follow distance increased "
                      f"(setpoint={pid.setpoint_area:.2f})")

            elif key == ord('-'):
                pid.setpoint_area = min(
                    0.50, pid.setpoint_area + 0.01)
                print(f"\n[PID] Follow distance decreased "
                      f"(setpoint={pid.setpoint_area:.2f})")
